...
class UploadApi extends Api {
    ...
    public function init() {
        $this->logic = D('Upload/Upload', 'Logic');
    }
    ...
    public function oneImage($config = array(), $file = '',$arr=array()) {
        return $this->logic->uploadOne($config,$file,$arr);
    }
    ...
    public function manyImage($config = array(),$arr=array()) {
        return $this->logic->uploadMany($config,$arr);
    }
    ...
    public function base64Upload($baseData,$ext='png',$config=array(),$arr=array()){
        return $this->logic->Base64Upload($baseData,$ext,$config,$arr);
    }
}