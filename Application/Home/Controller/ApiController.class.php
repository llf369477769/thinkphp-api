...
//            }
            ...
            /* 验证用户token */
            $verifyToken = D('Token')->authToken($this->header['USER-TOKEN']);
            ...
            if (!isset($verifyToken['token_decode']['uid'])) {
                Response::error(1003);
                return false;
            }else if(!empty($this->param['uid'])){
                if($verifyToken['token_decode']['uid'] != $this->param['uid']){
                    Response::error(1003);
                    return false;
                }
            }
            ...
            $this->userId = $verifyToken['token_decode']['uid']; //定义用户id

            ApiLog::setUserInfo($verifyToken);
        }
    }
